...
int Vstart;
int Vgoal;
#define MAXRESULT (MAXVERTICES/128)
struct result {
  int id;
  int dist;
  int stop; /* 0:continue, 1:stop */
...
#ifdef CUDA
__global__ void endof_loop(struct vertex **vhashtbl, int Vstart, int Vgoal, struct result *result)
{
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
#else
void endof_loop(int tid, struct vertex **vhashtbl, int Vstart, int Vgoal, struct result *result)
{
#endif
  ...
  if (tid == 0) {
    min_vp = search_vertex(vhashtbl, Vgoal);
    if (!min_vp || !min_vp->parent) {
      result[0].id = min_vp->id;
      result[0].dist = min_vp->total_distance;
      result[0].stop = 1;
    }
    else {
      i = 0;
      ...
      do {
	result[i].id = min_vp->id;
	result[i].dist = min_vp->total_distance;
	result[i].stop = 0;
	min_vp = min_vp->parent;
	/*printf("->%d(%d)", min_vp->id, min_vp->total_distance);*/
	i++;
      } while (result[i-1].id != Vstart);
      i--;
      result[i].stop = 1;
    }
  }
...
#ifdef CUDA
  endof_loop<<<GPUBL,GPUTH>>>(vhashtbl, Vstart, Vgoal, result);
#else
  for (i=0; i<THNUM; i++)
    endof_loop(i, vhashtbl, Vstart, Vgoal, result);
#endif

  if (cudaSuccess != cudaMemcpy(h_result, result, sizeof(struct result) * MAXRESULT, cudaMemcpyDeviceToHost)) { printf("can't cudaMemcpy5\n"); exit(1); }

  i = 0;
  do {
    if (i>0) printf("->");
    printf("%d(%d)", h_result[i].id, h_result[i].dist);
  } while (!h_result[i++].stop);
  printf("\n");
