     ...
     char **argv;
{
  ...
  for (i=0; i<image_HT; i++) {
    for (j=0; j<image_WD; j++) {
      ACCI[i*image_WD+j] = i*image_WD + j;
    }
  }
...
//EMAX5A begin x1 mapdist=0
  while (loop--) {                                                   /* mapped to WHILE() on BR[15][0][0] stage#0 */
    ...
    mop(OP_LDR,  3, &BR[3][0][1],  (Ull)yin_pm, x, MSK_D0, (Ull)yin_p, 320, 0, 0, (Ull)NULL, 320);        /* stage#3 */
    mop(OP_LDR,  3, &BR[3][0][0],  (Ull)yin_pp, x, MSK_D0, (Ull)yin_p, 320, 0, 0, (Ull)NULL, 320);        /* stage#3 */
    exe(OP_FMA, &r10, BR[1][0][0], EXP_H3210,  BR[2][0][0], EXP_H3210,  BR[3][0][0], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#4 */
    exe(OP_FMA, &r11, BR[1][0][1], EXP_H3210,  BR[2][0][1], EXP_H3210,  BR[3][0][1], EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL); /* stage#4 */
    exe(OP_FAD, &r12,  r10, EXP_H3210,  r11, EXP_H3210, 0LL, EXP_H3210, OP_NOP, 0LL, OP_NOP, 0LL);     /* stage#5 */
    mop(OP_STR,  3, &r12, (Ull)(yout_z1++), 0LL, MSK_D0, (Ull)yout_z2, 316, 0, 0, (Ull)NULL, 316);            /* stage#5 */
  }
//EMAX5A end
