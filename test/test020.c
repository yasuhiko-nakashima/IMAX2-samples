...
#define back_g_ker_core1(b, o, i) \
  ...
  mop(OP_LDWR,  1, &BR[b][1][1],       (Ull)op0[o],    oofs,        MSK_W0,    (Ull)ot0[o],    Mlen,      0,      0,   (Ull)NULL,   Mlen);   /* stage#2 */\
  mop(OP_LDWR,  1, &BR[b][2][1],       (Ull)ip0[i],    iofs,        MSK_W1,    (Ull)it0[i],    IMXlen,    0,      0,   (Ull)NULL,   IMXlen); /* stage#2 IMXlenが大きいのでLMM*2使用 */\
  exe(OP_NOP,      &AR[b][0], 0LL,     EXP_H3210,      0LL,         EXP_H3210, 0LL,            EXP_H3210, OP_NOP, 0LL, OP_NOP,      0LL);    /* stage#2 (dummy to set target location) */\
  mop(OP_LDWR,  1, &b00[o][i],         (Ull)kp0[o][i], 0LL,         MSK_W0,    (Ull)kp0[o][i], 1LL,       0,      1,   (Ull)NULL,   1LL);    /* stage#2 foldはunit[0]に要指定 */\
...
#define back_g_ker_core1_host(b, o, i) \
  mop(OP_LDWR,  1, &BR[b][1][1],       (Ull)op0[o],    oofs,        MSK_W0,    (Ull)ot0[o],    Mlen,      0,      0,   (Ull)NULL,   Mlen);   /* stage#2 */\
  mop(OP_LDWR,  1, &BR[b][2][1],       (Ull)ip0[i],    iofs,        MSK_W1,    (Ull)it0[i],    IMXlen,    0,      0,   (Ull)NULL,   IMXlen); /* stage#2 IMXlenが大きいのでLMM*2使用 */\
  mop(OP_LDWR,  1, &b00[o][i],         (Ull)kp1[o][i], 0LL,         MSK_W0,    (Ull)kp1[o][i], 1LL,       0,      1,   (Ull)NULL,   1LL);    /* stage#2 foldはunit[0]に要指定 */\
  exe(OP_FMA,      &b00[o][i], b00[o][i], EXP_H3210,   BR[b][2][1], EXP_H3210, BR[b][1][1],    EXP_H3210, OP_NOP, 0LL, OP_NOP,      0LL);    /* stage#2 */\
