...
#define MAXBUF 1024
  ...
  if ((fp = fopen(fname, "r")) == NULL) {
    printf("can't open cluster_info %s\n", fname);
    exit(1);
  }
  ...
  while (fgets(buf, MAXBUF, fp)) {
    if (!strncmp("cluster", buf, 7)) {
      ...
      if (vp = search_vertex(v)) {
	switch (k) {
	case 0:	vp->color = 0x00808080; break;
	case 1:	vp->color = 0x000000ff; break;
	case 2:	vp->color = 0x0000ff00; break;
	case 3:	vp->color = 0x0000ffff; break;
	case 4:	vp->color = 0x00ff0000; break;
	case 5:	vp->color = 0x00ff00ff; break;
	case 6:	vp->color = 0x00ffff00; break;
	case 7:	vp->color = 0x00ffffff; break;
	default:vp->color = 0x00808080; break;
	}
      }
    }
  }
     ...
     char **argv;
{
  ...
  if (argc != 3) {
    printf("usage: %s <file> <cluster_info>\n", *argv);
    exit(1);
  }
