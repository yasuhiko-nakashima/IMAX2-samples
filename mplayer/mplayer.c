...
static int generate_video_frame(sh_video_t *sh_video, demux_stream_t *d_video)
{
	...
	if (decoded_frame) {
#if 1
	  mp_image_t *mp = decoded_frame; /* YUV420 */
	  int procimage();
	  procimage(mp->imgfmt, mp->width, mp->height, mp->num_planes,
		    (char*)(mp->planes[0]), (char*)(mp->planes[1]), (char*)(mp->planes[2]),
